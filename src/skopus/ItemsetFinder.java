...
public class ItemsetFinder {
	...
	private PriorityQueue<ItemsetRec> pqMItemsetTopk; // top K result sequential patterns
	...
	public ItemsetFinder() {
		pqMItemsetTopk = new PriorityQueue<ItemsetRec>(GlobalData.nK,
				new ItemsetAscSortByValue());
...
//		aryMCombineTemplate = new CombinTemplate[GlobalData.nSampleMaxLength + 1][(int) (GlobalData.nSampleMaxLength / 2) + 2];
	...
	/*
	// 20141016��ʹ��opus��˼·���Բ�����ĺ������и�д
	// �ӱ�ѡitemsets�У���������Ȥ�ļ���
	public void generateResultItemsets() {
		// 1 ��ʼ��sequence pattern set��single item����available item sets
		ItemQClass iqAvailItems = new ItemQClass();
		for (int i = 0; i < GlobalData.nNumOfItems; i++) {
			iqAvailItems.insert(i);
		}

		// 2 ���ɺ�ѡ���кͺ�ѡ�
//		ArrayList<ItemsetRec> alCandidateSequenceSet = new ArrayList<ItemsetRec>();
//		ArrayList<ItemQClass> alCandidateItemSet = new ArrayList<ItemQClass>();
		for (int i = 0; i < iqAvailItems.size(); i++) {
			ItemsetRec r = new ItemsetRec();
			r.add(iqAvailItems.get(i).item);
//			alCandidateSequenceSet.add(r);
			
			ItemQClass iqItems = new ItemQClass();
			for (int j = 0; j <= i; j++) {
				iqItems.insert(iqAvailItems.get(j).item, iqAvailItems.get(j).covernumber);
			}
//			alCandidateItemSet.add(iqItems);
			
			opus(r, iqItems);
		}//for (int i = 0; i < iqAvailItems.size(); i++) 

		// 3 �ݹ����opus
//		if(alCandidateSequenceSet.size() != alCandidateItemSet.size()){
//			System.out.println("Error: ");
//			return;
//		}
		
//		assert(alCandidateSequenceSet.size() == alCandidateItemSet.size());
//		for (int ix = 0; ix < alCandidateSequenceSet.size(); ix++) {
//			opus(alCandidateSequenceSet.get(ix), alCandidateItemSet.get(ix));
//		}
	...
	public void generateResultItemsets() {
		...
		System.out.println("Content topK "+this.toString());
		...
		for (int i = 0; i < iqAvailSymbols.size(); i++) {
			...
			//2.2�ݹ����opus
			opus(r, iqAvailSymbols);
		}
	}
	
	
	//20150121��ʹ��Geoff����ȷ˼·��д
	private void opus(final ItemsetRec irCandidateSequence,
		...
		for(int ix = 0; ix < iqNewAvialItems.size(); ix++){
			...
			opus(irNewSequence, iqNewAvialItems);
		}
	...
	public String toString() {
		...
		while (itItemsetsResult.hasNext()) {
			alItemsetsResult.add(new ItemsetRec((ItemsetRec) itItemsetsResult
					.next()));
		}
		Collections.sort(alItemsetsResult, new ItemsetDecSortByValue());
	...
	public void outputResult(String strFilename) {
		...
		GlobalOper.appendFileContent(strFilename, 
				"the number of symbols: " + GlobalData.nNumOfItems + "\n");
		GlobalOper.appendFileContent(strFilename, 
				"the number of sequences: " + GlobalData.nNumOfSequence + "\n");
		GlobalOper.appendFileContent(strFilename, 
				"the average length of the sequences: " + GlobalData.dSampleAverageLength + "\n");
		GlobalOper.appendFileContent(strFilename, 
				"the Maximum length of the sequences: " + GlobalData.nSampleMaxLength + "\n");
	}
...
//		int numSupport = 0;
//		for (numSupport = 0; numSupport < iqAvailSymbols.size(); numSupport++) {
//			String e1 = GlobalData.alItemName.get(iqAvailSymbols.get(numSupport).item);
//			if(e1.equals("support"))break;
//		}
//		int numVector = 0;
//		for (numVector = 0; numVector < iqAvailSymbols.size(); numVector++) {
//			String e1 = GlobalData.alItemName.get(iqAvailSymbols.get(numVector).item);
//			if(e1.equals("vector"))break;
//		}
//		
//		int numMachin= 0;
//		for (numMachin = 0; numMachin < iqAvailSymbols.size(); numMachin++) {
//			String e1 = GlobalData.alItemName.get(iqAvailSymbols.get(numMachin).item);
//			if(e1.equals("machin"))break;
//		}
//		pattern.add(iqAvailSymbols.get(numSupport).item);
//		pattern.add(iqAvailSymbols.get(numVector).item);
//		pattern.add(iqAvailSymbols.get(numMachin).item);
//		evaluateSequencePattern(pattern);
//		System.out.println(pattern);
//		System.out.println("count="+pattern.count);
//		System.out.println("int="+pattern.value);
//		System.exit(0);
